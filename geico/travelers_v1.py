...
def auto_search(browser):
	...
	browser.get("https://www.travelers.com/")
	#browser.maximize_window()

	# customer
	wait.until(EC.element_to_be_clickable((By.ID,"zipCode")))
	browser.find_element_by_id("zipCode").send_keys(zipcode)
	browser.find_element_by_id("button-get-quote").click()
	...
	browser.find_element_by_id("streetAddr1").send_keys(street)
	browser.find_element_by_id("txtUnit").send_keys(apt)
	browser.find_element_by_xpath("//div[contains(@class,'buttons')]//input"").click()
	browser.find_element_by_name("dobMonth").send_keys(day_dob)
	browser.find_element_by_name("dobDay").send_keys(month_dob)
	browser.find_element_by_name("dobYear").send_keys(year_dob)
