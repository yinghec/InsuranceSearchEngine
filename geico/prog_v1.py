...
def auto_search(browser):
	...
	browser.get("https://www.progressive.com/auto/")
	#browser.maximize_window()

	# customer
	wait.until(EC.element_to_be_clickable((By.ID,"zipCode")))
	browser.find_element_by_name("ZipCode").send_keys(zipcode)
	browser.find_element_by_id("qsButton").click()
	wait.until(EC.element_to_be_clickable((By.ID, "NameAndAddressFormModel_FirstName_Value")))
	browser.find_element_by_name("NameAndAddressFormModel.FirstName.Value").send_keys(firstName)
	browser.find_element_by_name("NameAndAddressFormModel.LastName.Value").send_keys(lastName)
	browser.find_element_by_name("NameAndAddressFormModel.MailingAddress.Value").send_keys(street)
	browser.find_element_by_name("NameAndAddressFormModel.ApartmentUnit.Value").send_keys(apt)
	browser.find_element_by_name("NameAndAddressFormModel.DateOfBirth.Month").send_keys(month_dob)
	browser.find_element_by_name("NameAndAddressFormModel.DateOfBirth.Day").send_keys(day_dob)
	browser.find_element_by_name("NameAndAddressFormModel.DateOfBirth.Year").send_keys(year_dob)
	...
	sleep(5)
	browser.find_element_by_xpath("//div[contains(@class,'buttons')]//input").click();
	...
	#submit = browser.find_element_by_id("next")
	#for option in browser.find_element_by_tag_name('input')
	#	if option.text == ""
